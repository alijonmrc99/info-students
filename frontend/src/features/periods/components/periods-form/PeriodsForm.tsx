...
import { periodSlice } from "../../slices";
...
export const PeriodsForm: FC<{ level?: number, parentId?: ID }> = () => {
    const { contexHolder, control, setValue, isLoading, handleSendForm } = usePlace();
    const { t } = useTranslation();
    const { id } = useParams()
    const dispatch = useAppDispatch();
    const { result: place } = useAppSelector(state => state.period);

    useEffect(() => {
        if (id) {
            dispatch(fetchOnePeriod(id))
        }
    }, [t])
    ...
    useEffect(() => {
        if (place) {
            setValue('id', id)
            setValue('nameUz', place.nameUz)
            setValue('nameEn', place.nameEn)
        }
    ...
    return <div className="place-form">
        <Divider orientation="left">
            <h4>{place?.name ? `${place.name.slice(0, 100)} ...` : `${t('new')} ${t('period').toLowerCase()} ${t('add').toLowerCase()} `}</h4>
        </Divider>
