...
interface ImageUploaderProps {
    ...
    oldImages: { id: ID, path: string }[]
}

export const ImageUploader: React.FC<ImageUploaderProps> = ({ oldImages, setIsLoaded, accept = "image/*", customStyle = { height: "40px" }, label, path, multiple = false, setValue, name }) => {
    ...
    const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                console.log("result", result.result.imageIds);

                setValue && setValue(name, [...result.result.imageIds],)
